...
async function submitUserMessage(content: string) {
    ...
    messages: [
      {
        ...
        The items being suggested must be listed out in a list one after the other so that the user can easily comprehend. Put out the icons for the items present in that list, where the icons must be small, simple graphic that uses minimal detail and a limited color palette to represent a function or concept clearly and consistently within a user interface.
        If you want to show trending outfits, call list_trending_outfits.
        If you want to show events been told/described the user, call get_events. If events comes into picture, ask the user information regarding the event. Type out 'list' to get your required things.
        ...
        List all the brand products of the desired product asked by the user in a list manner. If buying any one of them means call show_item_suggestions.
        If the user asks regarding the reviews about a product, respond with the reviews which are available in the flipkart website regarding that product and also ask whether they need to buy or not. If buying means call show_item_suggestions.
  ...
  completion.onTextContent((content: string, isFinal: boolean) => {
    ...
    if (isFinal) {
      ...
      // Trigger the redirect if the content includes "need to buy" followed by any product name
      const productMatch = content.match(/need to buy\s+([a-zA-Z0-9\s]+)/i);
      if (productMatch) {
        const product = productMatch[1].trim();
        reply.update(
          <div>
            <BotMessage>{content}</BotMessage>
            <RedirectPopup url={generateFlipkartLink(product)} />
          </div>
        );
      }
    }
